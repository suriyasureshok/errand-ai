...
async def test_patch_generator_execute(mock_config: Config):
    ...
    expected_recommendation = PatchRecommendation(
        ...
        patches=[
            SearchReplacePatch(
                file_path="app.py", search_block="a - b", replace_block="a + b"
            )
        ],
    ...
    package = ContextPackage(
        ...
        collected_files=[
            ContextFile(path="app.py", content="def add(a,b): return a - b")
        ],
    ...
    assert (
        patch_file.read_text(encoding="utf-8") == expected_recommendation.unified_diff
    )
