...
async def test_refactor_agent_execute(mock_config: Config):
    ...
    request = RefactorRequest(
        package=ContextPackage(error_type="Err", error_summary="Sum", collected_files=[], relevant_tests=[], related_modules=[]),
        previous_recommendation=PatchRecommendation(root_cause="old", proposed_solution="old", patches=[], unified_diff="old diff"),
